    ...
    const updatePromises = Object.keys(notifications).map(async (id) => {
      ...
      if (!isRead) {
        ...
        try {
          ...
          const isVerifiedRead = verifyData?.read === true || verifyData?.read === "true" || verifyData?.read === 1;
          
          if (isVerifiedRead) {
            updatedCount++;
            return true;
          } else {
            // Still count as updated even if verification fails (timing issue)
            updatedCount++;
            if (process.env.NODE_ENV === 'development') {
              console.warn(`⚠️ Verification failed for notification ${id}, but continuing`);
            }
            return true;
          }
        } catch (updateError) {
